...
*Generated: 2026-08-26 · Version: v2.16.6 (condition_note hover tooltip) · Live at: gcgeartracker.com*
...
## Current State: v2.16.6 — condition_note hover tooltip, pending push (v2.16.5 pushed 2026-08-26, superseded by this push)
...
- **v2.16.6** — **condition_note: "Condition & Details" hover tooltip.** Extracts the freeform staff note (e.g. "Includes Hardshell Case") that sometimes follows a "Condition & Details" marker in Algolia's `longDescription` field, via new `_extract_condition_note()`. Wired through `parse_products()` → `_cat_cache` → `_build_base_item_list()` → `/api/browse` → and the SSE small-scan `fmt(p)` path — same 5-location pattern as the `is_vintage` flag. Frontend: desktop table only (`_buildRowHtml` in `static/gc.js`), Condition cell gets a `title=` tooltip + dotted-underline hint (`.cond-has-note` in `gc.css`) when a note exists; mobile card/list views untouched, per Chuck's request. Empty for existing cached items until their next scan (same rollout caveat as `is_vintage`). `gc_tracker_app.py` + `static/gc.js` + `static/gc.css`. See HANDOFF.md for full detail.
